...
export default function ProductoDetalleClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const [{ data: prod }, { data: rev }] = await Promise.all([
          api.get(`/productos/${id}`),
          api.get(`/resenas/vendedor/${id}`).catch(() => ({ data: [] })),
        ]);
        setProducto(prod);
        // fetch resenas del vendedor
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-12">
        ...
        <div>
          ...
          <p className="text-4xl font-bold text-sabana-azul mb-6">${producto.precio.toLocaleString("es-CO")}</p>
          <p className="text-gray-600 leading-relaxed mb-8 whitespace-pre-line">{producto.descripcion}</p>
          ...
          <Link href={`/perfil/${producto.vendedor._id}`} className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl hover:bg-gray-100 transition-colors mb-4">
            <div className="w-14 h-14 rounded-full bg-sabana-azul flex items-center justify-center text-white font-bold text-xl overflow-hidden flex-shrink-0">
              {producto.vendedor.foto ? <Image src={producto.vendedor.foto} alt={producto.vendedor.nombre} width={56} height={56} className="object-cover w-full h-full" /> : producto.vendedor.nombre.charAt(0).toUpperCase()}
            </div>
            <div>
          ...
          {usuario?.id === producto.vendedor._id && (
            <div className="flex gap-3 mb-4">
              <button
              onClick={() => router.push(`/producto/${producto._id}/editar`)}
